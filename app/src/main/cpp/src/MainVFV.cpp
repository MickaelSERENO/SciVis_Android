        ...
        if(ptDescSize > 1)
        {
            for(uint32_t i = 0; i < ptDescSize-1; i++)
            {
                for(uint32_t j = i+1; j < ptDescSize; j++)
                {
                    ...
                    uint32_t maxHistogramVal = std::numeric_limits<float>::min();
                    #ifdef _OPENMP
                    #pragma omp parallel for reduction(max:maxHistogramVal)
                    #endif
                    for(uint32_t k = 0; k < HISTOGRAM_WIDTH*HISTOGRAM_HEIGHT; k++)
                        maxHistogramVal = std::max(maxHistogramVal, histogram[k]);
                    ...
                    #endif
                    for(uint32_t k = 0; k < HISTOGRAM_WIDTH*HISTOGRAM_HEIGHT; k++)
                        (*(float*)(histogram+k)) = (float)histogram[k]/maxHistogramVal;
                    ...
                    runOnMainThread([this, dataset, histogram, i, j]()
                    {
                        ...
                        //TEST
                        m_notConnectedTextureMtl->bindTexture(text->getTextureID(), 2, 0);
