...
namespace sereno {
    MainVFV::MainVFV(GLSurfaceViewData *surfaceData, ANativeWindow *nativeWindow, VFVData *mainData)
            : m_surfaceData(surfaceData), m_mainData(mainData) {
        ...
        m_vfMtl = new Material(&surfaceData->renderer,
                               surfaceData->renderer.getShader("vectorField"));
        ...
        m_colorPhongMtl = new PhongMaterial(&surfaceData->renderer, Color::BLUE_COLOR, 0.3f, 0.7f,
                                            0.1f, 100);
        ...
        for (int i = 0; i < 8; i++) {
            ...
            new(m_3dImageManipGO + i) DefaultGameObject(NULL, &surfaceData->renderer,
                                                        m_3dTextureMtl + i, m_gpuTexVBO);
        }
        ...
        m_notConnectedGO = new DefaultGameObject(NULL, &surfaceData->renderer,
                                                 m_notConnectedTextureMtl, m_gpuTexVBO);
        ...
        m_currentVisFBOGO = new DefaultGameObject(NULL, &surfaceData->renderer, m_currentVisFBOMtl,
                                                  m_gpuTexVBO);
    ...
    void MainVFV::placeWidgets() {
        ...
        m_3dImageManipGO[LEFT_IMAGE].setPosition(
                glm::vec3(-1.0f + widgetWidth * 0.5f, 0.0f, -1.0f));

        m_3dImageManipGO[RIGHT_IMAGE].setScale(glm::vec3(widgetWidth, widgetHeight, 1.0f));
        m_3dImageManipGO[RIGHT_IMAGE].setPosition(
                glm::vec3(1.0f - widgetWidth * 0.5f, 0.0f, -1.0f));

        m_3dImageManipGO[TOP_IMAGE].setScale(
                glm::vec3(2.0f - 2.0 * widgetWidth, widgetWidth, 1.0f));
        m_3dImageManipGO[TOP_IMAGE].setPosition(glm::vec3(0.0f, ratio - widgetWidth * 0.5f, -1.0f));
        m_3dImageManipGO[BOTTOM_IMAGE].setScale(
                glm::vec3(2.0f - 2.0 * widgetWidth, widgetWidth, 1.0f));
        m_3dImageManipGO[BOTTOM_IMAGE].setPosition(
                glm::vec3(0.0f, -ratio + widgetWidth * 0.5f, -1.0f));
        ...
        m_3dImageManipGO[TOP_LEFT_IMAGE].setPosition(
                glm::vec3(-1.0f + widgetWidth * 0.5f, ratio - widgetWidth * 0.5f, -1.0));
        m_3dImageManipGO[TOP_RIGHT_IMAGE].setPosition(
                glm::vec3(1.0f - widgetWidth * 0.5f, ratio - widgetWidth * 0.5f, -1.0));

        m_3dImageManipGO[BOTTOM_LEFT_IMAGE].setPosition(
                glm::vec3(-1.0f + widgetWidth * 0.5f, -ratio + widgetWidth * 0.5f, -1.0));
        m_3dImageManipGO[BOTTOM_RIGHT_IMAGE].setPosition(
                glm::vec3(1.0f - widgetWidth * 0.5f, -ratio + widgetWidth * 0.5f, -1.0));
    ...
    void MainVFV::placeCamera(bool forceReset) {
        ...
        if (m_selecting) {
            m_surfaceData->renderer.setOrthographicMatrix(
                    -m_tabletScale * m_surfaceData->renderer.getWidth() / 2.0f,
                    m_tabletScale * m_surfaceData->renderer.getWidth() / 2.0f,
                    -m_tabletScale * ((float) m_surfaceData->renderer.getHeight()) / 2.0f,
                    m_tabletScale * ((float) m_surfaceData->renderer.getHeight()) / 2.0f,
                    -0.0f, 100.0f, false);
            m_surfaceData->renderer.getCameraTransformable().setPosition(m_tabletPos);
            m_surfaceData->renderer.getCameraTransformable().setRotate(
                    m_tabletRot * Quaternionf(glm::vec3(1.0, 0.0, 0.0), M_PI / 2.0f));
        } else if ((m_currentWidgetAction == NO_IMAGE && numberFinger <= 1) ||
                   !m_curSDCanBeModified) {
            //Cancel the animation
            if (m_inAnimation || forceReset) {
                m_animationTimer = 0;

                m_surfaceData->renderer.setOrthographicMatrix(-1.0f, 1.0f,
                                                              -((float) m_surfaceData->renderer.getHeight()) /
                                                              m_surfaceData->renderer.getWidth(),
                                                              ((float) m_surfaceData->renderer.getHeight()) /
                                                              m_surfaceData->renderer.getWidth(),
                                                              -10.0f, 10.0f, false);

                //Reset camera position
                m_surfaceData->renderer.getCameraTransformable().setPosition(
            ...
            if (!m_inAnimation || forceReset) {
                m_surfaceData->renderer.setPerspectiveMatrix(45.0f * 3.14f / 180.0f,
                                                             ((float) m_surfaceData->renderer.getWidth()) /
                                                             m_surfaceData->renderer.getHeight(),
                                                             0.1, 100.0f, false);
                if (m_currentVis) {
                    m_animationStartingPoint = m_currentVis->getPosition();
                    if (!findHeadsetCameraTransformation(&m_animationEndingPoint,
                                                         &m_animationRotation)) {
            ...
            if (m_currentVis && m_animationTimer <= MAX_CAMERA_ANIMATION_TIMER) {
                ...
                m_surfaceData->renderer.getCameraTransformable().setPosition(
                        ((float) m_animationTimer) / MAX_CAMERA_ANIMATION_TIMER * dir);
    ...
    void MainVFV::run() {
        m_surfaceData->renderer.setViewport(Rectangle2i(0, 0, m_surfaceData->renderer.getWidth(),
                                                        m_surfaceData->renderer.getHeight()));
        ...
        while (!m_surfaceData->isClosed()) {
            ...
            if (!m_currentVis || !visible)
                ...
                m_curSDCanBeModified = m_mainData->canSubDatasetBeModified(
                        m_currentVis->getModel());
            ...
            while (Event * event = m_surfaceData->pollEvent()) {
                switch (event->type) {
                    ...
                    case RESIZE:
                        //Redo the viewport
                        m_surfaceData->renderer.setViewport(
                                Rectangle2i(0, 0, m_surfaceData->renderer.getWidth(),
                                            m_surfaceData->renderer.getHeight()));
                        placeWidgets();
                        placeCamera(true);
                        break;
            ...
            if (visible && m_currentVis) {
                if (m_snapshotCnt == MAX_SNAPSHOT_COUNTER) {
                    ...
                    if (sd != NULL) {
                        ...
                        if ((curSnapshot == NULL || curSnapshot->width != snapWidth ||
                             curSnapshot->height != snapHeight)
                            && (snapWidth * snapHeight != 0)) {
                            Snapshot *newSnapshot = new Snapshot(snapWidth, snapHeight,
                                                                 (uint32_t *) malloc(
                                                                         sizeof(uint32_t) *
                        ...
                        //Read pixels
                        glReadPixels(0, 0, snapWidth, snapHeight, GL_RGBA, GL_UNSIGNED_BYTE,
                                     curSnapshot->pixels);
                ...
                for (auto &it : m_modelChanged) {
                    ...
                    if (m_currentVis && m_currentVis->getModel() == sd) {
                        ...
                        if (it.second.updateScale)
                            m_currentVis->setScale(glm::vec3(sd->getScale().x, sd->getScale().y,
                                                             sd->getScale().z));
                        break;
                    }
                }
            ...
            if (visible) {
                ...
                if (m_surfaceData->renderer.getCameraParams().w == 1.0 &&
                    !m_selecting) //Orthographic mode
                {
                    for (int i = 0; i < 8; i++)
        ...
        switch (m_currentWidgetAction) {
            ...
            case BOTTOM_LEFT_IMAGE: {
                ...
                float inv = (((event->y - event->startY) * (event->y - event->startY) +
                              (event->x - event->startX) * (event->x - event->startX)) >
                             ((event->startY - event->oldY) * (event->startY - event->oldY) +
                              (event->startX - event->oldX) * (event->startX - event->oldX))) ? 1.0f
                                                                                              : -1.0f;
                factor *= inv;
                ...
                currentScale.x = currentScale.y = currentScale.z = fmax(
                        (float) currentScale.x + factor * 2.0f, 0.0f);
                ...
                if (m_selecting) {
                    ...
                    if ((vec1.x != 0 || vec1.y != 0) &&
                        ...
                        float distanceAfter = sqrt((tc1->x - tc2->x) * (tc1->x - tc2->x) +
                                                   (tc1->y - tc2->y) * (tc1->y - tc2->y) *
                                                   m_surfaceData->renderer.getHeight() /
                                                   m_surfaceData->renderer.getWidth());

                        float distanceBefore = sqrt(
                                (tc1->oldX - tc2->oldX) * (tc1->oldX - tc2->oldX) +
                                (tc1->oldY - tc2->oldY) * (tc1->oldY - tc2->oldY) *
                                m_surfaceData->renderer.getHeight() /
                                m_surfaceData->renderer.getWidth());

                        float distanceOrigin = sqrt(
                                (tc1->startX - tc2->startX) * (tc1->startX - tc2->startX) +
                                (tc1->startY - tc2->startY) * (tc1->startY - tc2->startY) *
                                m_surfaceData->renderer.getHeight() /
                                m_surfaceData->renderer.getWidth());
                    ...
                    Quaternionf q = Quaternionf(leftVector, pitch) * Quaternionf(roll, 0, 0) *
                                    m_currentVis->getRotate();
        ...
        if (ptDescSize > 1) {
            for (uint32_t i = 0; i < ptDescSize - 1; i++) {
                for (uint32_t j = i + 1; j < ptDescSize; j++) {
                    uint32_t *histogram = (uint32_t *) malloc(
                            sizeof(uint32_t) * HISTOGRAM_WIDTH * HISTOGRAM_HEIGHT);
                    if (!dataset->create2DHistogram(histogram, HISTOGRAM_WIDTH, HISTOGRAM_HEIGHT,
                                                    dataset->getPointFieldDescs()[i].id,
                                                    dataset->getPointFieldDescs()[j].id)) {
                        LOG_WARNING("Could not create histogram between property %s and %s\n",
                                    dataset->getPointFieldDescs()[i].name.c_str(),
                                    dataset->getPointFieldDescs()[j].name.c_str());
                        loaded = false;
                        free(histogram);
                        continue;
                    }
...
#pragma omp parallel for
#endif
                        for (uint32_t k = 0; k < HISTOGRAM_WIDTH * HISTOGRAM_HEIGHT; k++)
                            ((float *) histogram)[k] = ((float) histogram[k]) /
                                                       (float) maxVal; //This work because sizeof(float) == sizeof(uint32_t)
                    }
                    ...
                    runOnMainThread([this, dataset, histogram, i, j]() {
                        ...
                        Texture2D *text = new Texture2D(HISTOGRAM_WIDTH, HISTOGRAM_HEIGHT,
                                                        histogram, GL_RED, GL_RED, GL_FLOAT);
                        ...
                        DefaultGameObject rawGO(NULL, &m_surfaceData->renderer, m_cpcpMtl,
                                                m_gpuTexVBO);
                        ...
                        void *pixels = malloc(
                                sizeof(float) * CPCP_TEXTURE_WIDTH * CPCP_TEXTURE_HEIGHT);
                        glReadPixels(0, 0, CPCP_TEXTURE_WIDTH, CPCP_TEXTURE_HEIGHT, GL_RED,
                                     GL_FLOAT, pixels);
...
#pragma omp parallel for reduction(max:maxVal)
                            ...
                            for (uint32_t k = 0; k < CPCP_TEXTURE_WIDTH * CPCP_TEXTURE_HEIGHT; k++)
                                maxVal = (((float *) pixels)[k] > maxVal ? ((float *) pixels)[k]
                                                                         : maxVal);
                        }
                        ...
                        DefaultGameObject go(NULL, &m_surfaceData->renderer, m_normalizeMtl,
                                             m_gpuTexVBO);
                        ...
                        hist2D.pcpTexture = std::shared_ptr<Texture2D>(
                                new Texture2D(redToGrayFBO.stealColorBuffer(), CPCP_TEXTURE_WIDTH,
                                              CPCP_TEXTURE_HEIGHT));
                        hist2D.ptFieldID1 = i;
                        hist2D.ptFieldID2 = j;

                        std::shared_ptr <DatasetHistograms> metaData = m_mainData->getDatasetHistograms(
                                m_mainData->getDatasetSharedPtr(dataset));
                        if (metaData.get())
                        ...
                        glReadPixels(0, 0, CPCP_TEXTURE_WIDTH, CPCP_TEXTURE_HEIGHT, GL_RGBA,
                                     GL_UNSIGNED_BYTE,
                                     pixels); //This work because sizeof(float) = =sizeof(uint32_t)
                        glBindFramebuffer(GL_FRAMEBUFFER, curFBO);
                        m_mainData->sendCPCPTexture(m_mainData->getDatasetSharedPtr(dataset),
                                                    (uint32_t *) pixels, CPCP_TEXTURE_WIDTH,
                                                    CPCP_TEXTURE_HEIGHT, i, j);
        ...
        for (uint32_t i = 0; i < ptDescSize; i++) {
            ...
            if (!dataset->create1DHistogram(histogram, HISTOGRAM_WIDTH,
                                            dataset->getPointFieldDescs()[i].id)) {
...
#pragma omp parallel for
            ...
            runOnMainThread([this, dataset, histogram, i]() {
                ...
                Texture2D *text = new Texture2D(HISTOGRAM_WIDTH, 1, histogram, GL_RED, GL_RED,
                                                GL_FLOAT);
                ...
                m_mainData->send1DHistogram(m_mainData->getDatasetSharedPtr(dataset),
                                            (float *) histogram, HISTOGRAM_WIDTH, i);
        ...
        while (VFVEvent * event = m_mainData->pollEvent()) {
            switch (event->getType()) {
                ...
                case VFV_ADD_CLOUD_POINT_DATA: {
                    ...
                    event->cloudPointData.dataset->loadValues(
                            [](Dataset *dataset, uint32_t status, void *data) {
                                ((MainVFV *) data)->onLoadCloudPointDataset(
                                        reinterpret_cast<CloudPointDataset *>(dataset), status);
                            }, this);
                    break;
                }
                ...
                case VFV_ADD_VTK_DATA: {
                    m_vtkStructuredGridPoints.push_back(
                            new VTKStructuredGridPointSciVis(&m_surfaceData->renderer,
                                                             m_colorGridMtl, event->vtkData.dataset,
                                                             VTK_STRUCTURED_POINT_VIS_DENSITY));
                    ...
                    event->vtkData.dataset->loadValues(
                            [](Dataset *dataset, uint32_t status, void *data) {
                                ((MainVFV *) data)->onLoadVTKDataset(
                                        reinterpret_cast<VTKDataset *>(dataset), status);
                            }, this);
                ...
                case VFV_REMOVE_DATASET: {
                    ...
                    for (auto it = m_vtkStructuredGridPoints.begin();
                         it != m_vtkStructuredGridPoints.end(); it++) {
                    ...
                    for (auto it = m_cloudPointDatasets.begin();
                         it != m_cloudPointDatasets.end(); it++) {
                ...
                case VFV_SET_ROTATION_DATA: {
                    addSubDataChangement(event->sdEvent.sd,
                                         SubDatasetChangement(false, true, false, false));
                    break;
                }

                case VFV_SET_POSITION_DATA: {
                    addSubDataChangement(event->sdEvent.sd,
                                         SubDatasetChangement(false, false, false, true));
                    break;
                }

                case VFV_SET_SCALE_DATA: {
                    addSubDataChangement(event->sdEvent.sd,
                                         SubDatasetChangement(false, false, true, false));
                    break;
                }

                case VFV_SET_TF_DATA: {
                    addSubDataChangement(event->sdEvent.sd,
                                         SubDatasetChangement(true, false, false, false));
                    break;
                }
                ...
                case VFV_SET_CURRENT_DATA: {
                    ...
                    if (event->sdEvent.sd)
                        addSubDataChangement(event->sdEvent.sd,
                                             SubDatasetChangement(true, true, true, true));
                    break;
                }
    ...
    void
    MainVFV::addSubDataChangement(const SubDataset *sd, const SubDatasetChangement &sdChangement) {
        auto it = m_modelChanged.find(sd);
        if (it == m_modelChanged.end())
            m_modelChanged.insert(
                    std::pair<const SubDataset *, SubDatasetChangement>(sd, sdChangement));
        else
    ...
    SciVis *MainVFV::createVisualization(SubDataset *sd) {
        ...
        for (auto it : m_vtkStructuredGridPoints) {
            if (it->dataset.get() == sd->getParent()) {
                ...
                VTKStructuredGridPointGameObject *go = new VTKStructuredGridPointGameObject(NULL,
                                                                                            &m_surfaceData->renderer,
                                                                                            m_colorGridMtl,
                                                                                            it->vbo,
                                                                                            sd);
        ...
        for (auto it : m_cloudPointDatasets) {
            if (it.get() == sd->getParent()) {
                CloudPointGameObject *cp = new CloudPointGameObject(NULL, &m_surfaceData->renderer,
                                                                    m_cloudPointMtl, it, sd);
        ...
        if (added) {
            ...
            //Update the snapshot
            std::pair < SciVis * , std::shared_ptr < Snapshot
                    >> snap(m_sciVis.back(), std::shared_ptr<Snapshot>(nullptr));
            m_snapshots.insert(snap);
            m_sciVis.back()->getModel()->setSnapshot(snap.second);

            addSubDataChangement(m_sciVis.back()->getModel(),
                                 SubDatasetChangement(true, true, true, true));
    ...
    void MainVFV::removeSciVis(SciVis *sciVis) {
        ...
        //Check vector field
        for (std::vector<VectorField *>::iterator it = m_vectorFields.begin();
             it != m_vectorFields.end(); it++)
            if ((*it) == sciVis) {
        ...
        for (std::vector<DefaultSciVis *>::iterator it = m_defaultSciVis.begin();
             it != m_defaultSciVis.end(); it++)
            if ((*it) == sciVis) {
