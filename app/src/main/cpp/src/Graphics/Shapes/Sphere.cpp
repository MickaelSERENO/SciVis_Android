...
namespace sereno
{
    Sphere::Sphere(uint32_t nbLatitude, uint32_t nbLongitude)
    {
        ...
        for(unsigned int i=0; i < nbLongitude; i++)
        {
            ...
            {
                unsigned int o[] = {i*(nbLatitude) + j, ((i+1)*(nbLatitude))%nbLongitude + (j+1)%nbLatitude, (i+1)*(nbLatitude)%nbLongitude+j,
                                    i*(nbLatitude) + j, i*(nbLatitude) + (j+1)%nbLatitude, (i+1)*(nbLatitude)%nbLongitude + (j+1)%nbLatitude};
